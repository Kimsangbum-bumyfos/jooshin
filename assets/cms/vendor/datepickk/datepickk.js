...
function Datepickk(args) {
	...
	// dhshin 2018.05.14
	// 추가 - 설정된 일 수 만큼 날짜를 모두 선택할 경우 등록된 함수로 반환
	var onSelectedDate = null;
	...
	function onSelect(checked)
	{
        ...
        // 단일 날짜 선택일 경우
        if (rangeDate === 1) {
        	if (checked && onSelectedDate) {
	        	onSelectedDate.call(null, selectedDates);
	        }
        }

        // 2개의 날짜 선택의 경우
        else if (rangeDate >= 2) {
        	if (checked && selectedDates.length === 2) {
        		var days = (new Date(selectedDates[1]) - new Date(selectedDates[0]))/1000/60/60/24;
        		if (days > 0 && days <= rangeDate) {
	        		...
	        		unselectAll(true);
	        		onSelectedDate.call(null, selectedDates, true);
	        	}
	        	return;
	        }
	        else if (checked && selectedDates.length === 3) {
	        	// 색상 초기화
			...
			[].slice.call(element.querySelectorAll('.d-table input')).forEach(function (inputEl, i) {
				...
				if (date instanceof Date) {
					...
					if (_highlights.length > 0) {
						...
						// dhshin 2018.03.21
						// 동일날짜에 여가개의 highlight 가 걸려있다면 마지막 내용만 적용
						// 처음과 끝 날짜에는 border 옵션 추가
						var Color = '';
						var borderRadius = '';
						if (_highlights.length > 1) {
							...
							// }
							// bgColor += ');';
							bgColor = _highlights[_highlights.length-1].backgroundColor ? 'background:' + _highlights[_highlights.length-1].backgroundColor + ';' : '';
							legendIds += highlight.indexOf(_highlights[_highlights.length-1]);
							Color = _highlights[_highlights.length-1].color ? 'color:' + _highlights[_highlights.length-1].color + ';' : '';
							if (date.getTime() == _highlights[_highlights.length-1].dates[0].start.getTime()) {
								borderRadius = 'border-top-left-radius: 20px; border-bottom-left-radius: 20px;'
							}
							else if (date.getTime() == _highlights[_highlights.length-1].dates[0].end.getTime()) {
								borderRadius = 'border-top-right-radius: 20px; border-bottom-right-radius: 20px;'
							}

						} else {
							bgColor = _highlights[0].backgroundColor ? 'background:' + _highlights[0].backgroundColor + ';' : '';
							legendIds += highlight.indexOf(_highlights[0]);
							Color = _highlights[0].color ? 'color:' + _highlights[0].color + ';' : '';

							if (date.getTime() == _highlights[0].dates[0].start.getTime()) {
								borderRadius = 'border-top-left-radius: 20px; border-bottom-left-radius: 20px;'
							}
							else if (date.getTime() == _highlights[0].dates[0].end.getTime()) {
								borderRadius = 'border-top-right-radius: 20px; border-bottom-right-radius: 20px;'
							}
						}
						// var Color = _highlights[0].color ? 'color:' + _highlights[0].color + ';' : '';
						labelEl.setAttribute('style', bgColor + Color + borderRadius);
	...
	Object.defineProperties(that, {
		...
		// dhshin 2018.05.14
		// onSlect 내부 함수 처리
		// 선택 날짜만 반환하는 onSelectedDate 추가
		// 선택 날짜 범위 지정하는 rangeDate 추가
		"onSelectedDate": {
			set: function set(callback) {
		...
		"minDate": {
			...
			set: function set(x) {
				// dhshin 2018.05.14
				// 오늘부터 선택가능하게 new Date() 로만 설정할 경우 생성 시점에서의 시간이 지나가서 오늘 선택이 안됨
				// 특정 날짜를 넣어줌으로 해당 날짜의 00시 00분으로 설정하게 처리
				if (x) {
					var today = new Date(x);
    				x = new Date(today.getFullYear(), today.getMonth(), today.getDate());
				}
...
var template = '<div class="d-title"></div>' +
			   '<div class="d-calendar">' +
			       '<div class="d-header">' +
			            ...
			            '<i id="d-next"></i>' +
			        '</div>' +
			        '<div class="d-month-picker">' + '<div data-month="1">01</div>' + '<div data-month="2">02</div>' + '<div data-month="3">03</div>' + '<div data-month="4">04</div>' + '<div data-month="5">05</div>' + '<div data-month="6">06</div>' + '<div data-month="7">07</div>' + '<div data-month="8">08</div>' + '<div data-month="9">09</div>' + '<div data-month="10">10</div>' + '<div data-month="11">11</div>' + '<div data-month="12">12</div>' + '</div>' + '<div class="d-year-picker">' + '<div data-year="-5"></div>' + '<div data-year="-4"></div>' + '<div data-year="-3"></div>' + '<div data-year="-2"></div>' + '<div data-year="-1"></div>' + '<div data-year="0"></div>' + '<div data-year="1"></div>' + '<div data-year="2"></div>' + '<div data-year="3"></div>' + '<div data-year="4"></div>' + '<div data-year="5"></div>' + '</div>' + '<div class="d-weekdays"></div>' + '<div class="d-tables"></div>' + '</div>' + '<div class="d-legend"></div>' + '<button class="d-confirm"></button>' + '<div class="d-overlay"></div>';

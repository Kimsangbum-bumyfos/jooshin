...
class Peak extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('admin/pension_peak_m');
    ...
    public function lists() {
        ...
        // 페이징 주소
        $config['base_url']                 = $this->config->item('ADMIN_ROOT').'/pension/peak/lists'.$page_url;
        ...
        // 게시물 전체 개수
        $config['total_rows'] = $this->pension_peak_m->lists('tb_pension_peak', 'count','', '', $search_word);
...
//        // 한 페이지에 표시할 게시물 수
        ...
        $data['list'] = $this->pension_peak_m->lists('tb_pension_peak', '', $start, $config['per_page'], $search_word);

        $this->load->view('admin/pension/peak', $data);
    }
        ...
        if ($_POST){
            ...
            if($this->form_validation->run()) {
                $write_data = array(
                    ...
                    'table' => 'tb_pension_peak'
                );

                $result = $this->pension_peak_m->insert($write_data);
                ...
                $data['v'] = $v - 1;
                $this->load->view('admin/pension/peak_write', $data);
            }
        } else {
            // 쓰기 폼 view 호출
            $data['v'] = $v;
    ...
    function delete() {
        ...
        $return = $this->pension_peak_m->delete('tb_pension_peak', $this->uri->segment(5));
        $v = $this->input->get('v');
    ...
    function modify() {
        ...
        if ($_POST) {
            ...
            if($this->form_validation->run()) {
                ...
                $result = $this->pension_peak_m->modify($modify_data);
                ...
                $data['v'] = $v - 1;
                $data['views'] = $this->pension_peak_m->get_view('tb_pension_peak', $this->uri->segment(5));
                $this->load->view('admin/pension/peak_modify', $data);
            }
        } else {
            $data['v'] = $v ;
            $data['views'] = $this->pension_peak_m->get_view('tb_pension_peak', $this->uri->segment(5));

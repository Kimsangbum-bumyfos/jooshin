...
class Notice extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('admin/notice_m');
    ...
    public function lists() {
        ...
        // 페이징 주소
        $config['base_url']                 = $this->config->item('ADMIN_ROOT').'/board/notice/lists'.$page_url;
...
//
        // 게시물 전체 개수
        $config['total_rows'] = $this->notice_m->lists('tb_board', 'count','', '', $search_word);
...
//        // 한 페이지에 표시할 게시물 수
        ...
        $data['list'] = $this->notice_m->lists('tb_board', '', $start, $limit, $search_word);
        ...
        $this->load->view('admin/board/notice', $data);
    }
    ...
    function write() {
        ...
        if ($_POST) {
            ...
            if($this->form_validation->run()) {
                ...
                //------------파일 업로드 처리-----------//
                $config['upload_path'] = '.'.$this->config->item("UPLOAD_NOTICE");
                ...
                $write_data = array(
                    ...
                    'path' => $this->config->item("UPLOAD_NOTICE"),
                    'cke_key' => $this->input->post('k', TRUE),
                    'table' => "tb_board"
                );

                $result = $this->notice_m->insert($write_data);

                if ($result) {
                    //history($v);
                    replace($this->config->item('ADMIN_ROOT').'/board/notice');
                    exit;
                } else {
                ...
                $data['v'] = $v - 1;
                $this->load->view('admin/board/notice_write', $data);
            }
        } else {
            // 쓰기 폼 view 호출
            $data['hash'] = random_string('alnum', 32);
    ...
    function delete() {
        echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
        $data = $this->notice_m->get_view('tb_board', $this->uri->segment(5));
        $return = $this->notice_m->delete('tb_board', $this->uri->segment(5));
        $v = $this->input->get('v');
    ...
    function modify() {
        ...
        if ($_POST) {
            ...
            if($this->form_validation->run()) {
                ...
                $result = $this->notice_m->modify($modify_data);
            ...
            $data['v'] = $v - 1;
            $data['views'] = $this->notice_m->get_view('tb_board', $this->uri->segment(5));
            $this->load->view('admin/board/notice_modify', $data);
        } else {
            $data['v'] = $v;
            $data['views'] = $this->notice_m->get_view('tb_board', $this->uri->segment(5));
            $this->load->view('admin/board/notice_modify', $data);

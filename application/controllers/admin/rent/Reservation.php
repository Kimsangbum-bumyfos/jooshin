...
class Reservation extends CI_Controller {
    ...
    public function lists() {
        ...
        // 페이징 주소
        $config['base_url'] = $this->config->item('ADMIN_ROOT').'/rent/reservation/lists'.$page_url.'/page/';
        ...
        // 게시물 전체 개수
        $config['total_rows'] = $this->reservation_m->lists('tb_reservation', 'count','', '', $search_word);
        ...
        $data['list'] = $this->reservation_m->lists('tb_reservation', '', $start, $config['per_page'], $search_word);

        $this->load->view('admin/rent/reservation', $data);
    }
    ...
    function delete() {
        ...
        $return = $this->reservation_m->delete('tb_reservation', $this->uri->segment(5));
    ...
    function modify() {
        ...
        if ($_POST) {
            ...
            $result = $this->reservation_m->modify($modify_data);
            ...
            $data['views'] = $this->reservation_m->get_view('tb_reservation', $this->uri->segment(5));
            $this->load->view('admin/rent/reservation_modify', $data);
        }
    }
}

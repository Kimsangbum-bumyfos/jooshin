...
class Holiday extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('admin/holiday_m');
    ...
    public function lists() {
        ...
        // 페이징 주소
        $config['base_url']                 = $this->config->item('ADMIN_ROOT').'/rent/holiday/lists'.$page_url;
        ...
        // 게시물 전체 개수
        $config['total_rows'] = $this->holiday_m->lists('tb_holiday', 'count','', '', $search_word);
        ...
        $data['list'] = $this->holiday_m->lists('tb_holiday', '', $start, $config['per_page'], $search_word);

        $this->load->view('admin/rent/holiday', $data);
    }
    ...
    function write() {
        ...
        if ($_POST){
            ...
            if($check) {
                ...
                $result = $this->holiday_m->get_year_chk('tb_holiday', $this->input->post('year', TRUE));
                if($result==1){
                    alert_after_history('선택한 년도로 등록 된 휴무일이 있습니다', -1);
                    exit;
                }

                $result = $this->holiday_m->insert($write_data);
                ...
                $data['v'] = $v - 1;
                $this->load->view('admin/rent/holiday_write', $data);
            }
        } else {
            // 쓰기 폼 view 호출
            $data['v'] = $v;
    ...
    function delete() {
        ...
        $return = $this->holiday_m->delete('tb_holiday', $this->uri->segment(5));
    ...
    function modify() {
        ...
        if ($_POST) {
            ...
            if($check) {
                ...
                $result = $this->holiday_m->modify($modify_data);
                ...
                $data['v'] = $v - 1;
                $data['views'] = $this->holiday_m->get_view('tb_holiday', $this->uri->segment(5));
                $this->load->view('admin/rent/holiday_modify', $data);
            }
        } else {
            $data['v'] = $v ;
            $data['views'] = $this->holiday_m->get_view('tb_holiday', $this->uri->segment(5));

...
<head>
    ...
    <script>
        ...
        var ex_files = []; // 기존 파일 배열
        var cnt = 0;

        // file val
        var f_sel_files = []; // 파일 업로드 배열
        var f_ex_files = []; // 기존 파일 배열(law)
        var real_files = [];  // 기존 파일명 배열(real)
        var f_cnt = 0;
        ...
        $(document).ready(function() {
            ...
            textArea_1 = textArea_1.split('<br>').join("\r\n");
            $('#sub_title').val(textArea_1);


            cell.load(<?=$views->key_value;?>);    // 추가 항목 생성
            ...
            initDepth("<?= $views->depth_1?>", "<?= $views->depth_2?>", "<?= $views->depth_3 ?>"); // 카테고리 셋팅
            ...
            $("#write_btn").click(function() {
                ...
                // }
                    ...
                    for(var i =0; i<ex_files.length; i++){
                        if(parseInt(i)+parseInt(1) == ex_files.length)
                            dupList+= ex_files[i];
                        else
                            dupList += ex_files[i]+',';
                    }
                    ...
                    var f_dupList ='';
                    var dub_real_list = ''
                    for(var i =0; i<f_ex_files.length; i++){
                        if(parseInt(i)+parseInt(1) == f_ex_files.length){
                            f_dupList+= f_ex_files[i];
                            dub_real_list += real_files[i];
                        }
                        else{
                            f_dupList += f_ex_files[i]+',';
                            dub_real_list += real_files[i]+',';
                    ...
                    if(sel_files.length > 0 || $('#img_list').val() != dupList){
                        si = true;
                        fileUploadAction(dupList, 'products/product'); // 이미지업로드
                    }


                    if(f_sel_files.length > 0 || $('#file_list').val() != f_dupList){
                        // alert($('#file_list').val());
                        // alert(f_dupList);
                        sf = true;
                        fileUploadAction_product(f_dupList , dub_real_list); // 파일업로드
                    }
                    ...
                    <div class="section-body">
                        <div class="row">
                            <div class="content-area">
                                ...
                                <fieldset>
                                    <legend class="hidden-text">제품관리수정</legend>
                                    <table class="table table-typeB">
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="col-100">
                                                        ...
                                                        <input type="text" name="title" id="title" class="form" value="<?=$views->title;?>" placeholder="제품명을 입력하세요">
                                                    </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="col-100">
                                                        ...
                                                        <!-- <input type="text" name="sub_title" id="sub_title" class="form" value="<?=$views->sub_title;?>"placeholder="부가설명을 입력하세요"> -->

                                                        <textarea name="sub_title" id="sub_title" style="height:90px;" class="form" placeholder="부가설명을 입력하세요."><?=$views->sub_title;?></textarea>
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td class="clearfix">
                                                    <div class="col-33 fl-l">
                                                       ...
                                                       <select name="depth_1" id="depth_1" class="form product_depth_1">
                                                            <option value="계측장비" <?php if($views->depth_1 == '계측장비') echo "selected";?>>계측장비</option>
                                                            <option value="스트레인게이지" <?php if($views->depth_1 == '스트레인게이지') echo "selected";?>>스트레인게이지</option>
                                                            <option value="계측센서" <?php if($views->depth_1 == '계측센서') echo "selected";?>>계측센서</option>
                                                            <option value="비디오게이지" <?php if($views->depth_1 == '비디오게이지') echo "selected";?>>비디오게이지</option>
                                                            <option value="시험용치구" <?php if($views->depth_1 == '시험용치구') echo "selected";?>>시험용치구</option>
                                                            <option value="제작품" <?php if($views->depth_1 == '제작품') echo "selected";?>>제작품</option>
                                                       </select> 
                                                    </div>
                                                    <div class="col-33 fl-l">
                                                       <label for="depth_2" class="hidden-text">뎁스2</label>
                                                       <select name="depth_2" id="depth_2" class="form product_depth_2"></select> 
                                                    </div>
                                                    <div class="col-33 fl-l">
                                                       <label for="depth_3" class="hidden-text">뎁스3</label>
                                                       <select name="depth_3" id="depth_3" class="form product_depth_3"></select> 
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="col-100">
                                                        ...
                                                        <input type="text" name="model_name" id="model_name" class="form" value="<?=$views->model_name;?>" placeholder="모델명을 입력하세요">
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="col-100">
                                                        ...
                                                        <input type="text" name="manufacturer" id="manufacturer" class="form" value="<?=$views->manufacturer;?>" placeholder="제조사를 입력하세요">
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="col-100">
                                                        ...
                                                        <input type="text" name="type" id="type" class="form" value="<?=$views->type;?>" placeholder="타입을 입력하세요">
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="col-100">
                                                        ...
                                                        <input type="text" name="sensor_range" id="sensor_range" class="form" value="<?=$views->sensor_range;?>" placeholder="센서의 측정범위를 입력하세요">
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="col-100">
                                                        <label for="sensor_outline" class="hidden-text">측정범위</label>
                                                        <input type="text" name="sensor_outline" id="sensor_outline" class="form" value="<?=$views->sensor_outline;?>" placeholder="센서의 외경을 입력하세요">
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                            ...
                                            <!-- <tr>
                                                ...
                                                <td>
                                                    <div class="col-100">
                                                        <input type="checkbox" name="page_fix" id="page_fix" value="Y" <?php if($views->page_fix == 'Y') echo "checked"; ?> >
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="col-100">
                                                        <input type="checkbox" name="page_main" id="page_main" value="Y" <?php if($views->page_main == 'Y') echo "checked"; ?> >
                                                ...
                                                <td>
                                                    ...
                                                    <div class="file">
                                                        ...
                                                        <div class="col-100 m-t-8">
                                                            ...
                                                            <input type="file" name="userfile[]" id="input_file1" class="upload-hidden" accept="image/*">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <div class="col-100" style="display:none;">
                                                        <input type="text" name="img_list" id="img_list" value="<?= $views->img_list ?>">
                                                    </div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <th>미리보기</th>
                                                <td>
                                                    <div id="imgs_wrap" class="col-100">
                                                        <?php
                                                            $img_list = $views->img_list;
                                                            $img_arr = explode(',', $img_list);

                                                            if($img_list != '')
                                                            for ($i=0; $i<count($img_arr); $i++) {
                                                                echo  "<a href=\"javascript:void(0)\" id=\"img_id_7".$i."\" class=\"th-img-cnt-n\"><img id=\"".trim($img_arr[$i])."\" src=\"".base_url().$views->path.'/'.trim($img_arr[$i])."\"><span title=\"클릭시 삭제됩니다.\" onclick=\"deleteExImageAction(7".$i.");\" class=\"ico-file-delete\"></span></a>";
                                                            }
                                                        ?>
                                                        <script>
                                                            $('#imgs_wrap a').each(function(index) {
                                                                ex_files.push($('#imgs_wrap a').eq(index).find('img').attr('id'));
                                                            });
                                                        </script>
                                                    </div>
                                                </td>
                                            ...
                                            <!-- <tr>
                                                ...
                                                <td>
                                                    <div class="col-100">
                                                        ...
                                                        <div>
                                                            <textarea name="contents" id="contents" rows="30" style="width:100%;"><?=$views->contents?></textarea>
                                                            <script>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <div class="col-100" style="display:none;">
                                                        <input type="text" name="file_law_name" id="file_list" value="<?=$views->file_law_name;?>">
                                                        <input type="text" name="file_real_name" id="file_real_name" value="<?=$views->file_real_name;?>">
                                                    </div>
                                                </td>
                                            </tr>
                                            ...
                                            <tr>
                                                <th>파일목록</th>
                                                <td>
                                                    <div id="files_wrap" class="col-100">
                                                        <?php
                                                            $file_real_name = $views->file_real_name;
                                                            $file_arr = explode(',', $file_real_name);
                                                            $file_law_name = $views->file_law_name;
                                                            $file_law_arr = explode(',', $file_law_name);

                                                            if($file_real_name != '')
                                                            for ($i=0; $i<count($file_arr); $i++) {
                                                                echo "<p id=\"file_id_7".$i."\" class=\"th-file-cnt-n\"><a  href=\"".base_url()."uploads/products/".trim($file_law_arr[$i])."\" data-file-law=\"".trim($file_law_arr[$i])."\" data-file-real=\"".trim($file_arr[$i])."\" class=\"selFile\" target=\"blank\">".trim($file_arr[$i])."<span title=\"클릭시 삭제됩니다.\" onclick=\"event.preventDefault(); deleteFileAction(7".$i.");\" class=\"ico-file-delete\"></span></a></p>";
                                                            }
                                                        ?>
                                                        <script>
                                                            $('#files_wrap p').each(function(index) {
                                                                f_ex_files.push($('#files_wrap p').eq(index).find('a').data('fileLaw'));
                                                                real_files.push($('#files_wrap p').eq(index).find('a').data('fileReal'));
                                                            });
                                                        </script>
                                                    </div>
                                                </td>
                        ...
                        <div class="row">
                            <div class="content-area clearfix">
                                <div class="btn-group fl-l">
                                    <button class="btn typeA-gray" onclick="goDelete('<?=$this->config->item('ADMIN_ROOT'); ?>/products/product/delete/<?=$views->idx ?>?v=<?=$v?>')">삭제</button>
                                </div>

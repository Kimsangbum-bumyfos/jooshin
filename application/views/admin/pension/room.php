                                ...
                                <table class="table table-typeA">
                                    <tbody>
                                        <tr>
                                            ...
                                            <th class="m-hide">섬네일</th>
                                            <th>객실명</th>
                                            <th>정원/최대</th>
                                            <th class="m-hide">평수</th>
                                            <th>평일가격</th>
                                            <th>금요일가격</th>
                                            <th>주말가격</th>
                                            <th>게시여부</th>
                                        ...
                                        <tr>
                                            ...
                                            <td class="m-hide"><a href="<?= $this->config->item('ADMIN_BASE_URL'); ?>/pension/room/modify/<?= $lt->idx;?>?page=<?=$page?>" class="thumb-img"><img src="<?=base_url().$lt->path.'/'.$lt->p_thumb_img;?>"></a></td>
                                            <td><a href="<?= $this->config->item('ADMIN_BASE_URL'); ?>/pension/room/modify/<?= $lt->idx;?>?page=<?=$page?>" class="underline"><?= $lt->name; ?></a></td>
                                            <td><?= $lt->fixed_number ?>/<?= $lt->max_number ?></td>
                                            <td class="m-hide"><?= $lt->space ?></td>
                                            <td>
                                                <?php if($lt->off_season_normal_fee !=="") echo number_format($lt->off_season_normal_fee)."원"; ?>
                                            </td>
                                            <td>
                                                <?php if($lt->off_season_friday_fee !=="") echo number_format($lt->off_season_friday_fee)."원"; ?>
                                            </td>
                                            <td>
                                                <?php if($lt->off_season_weekend_fee !=="") echo number_format($lt->off_season_weekend_fee)."원"; ?>
                                            </td>
                                        ...
                                        <?php
                                            if($total_row==0){
                                                echo "<tr><td colspan='10' align='center'> 검색된 결과가 없습니다.</td></tr>";
                                            }    
                                        ?><!--// 검색결과 없는 경우 -->
                        ...
                        <div class="row">
                            <div class="content-area clearfix">
                                <div class="btn-group fl-r">
                                    <button onclick="goPage('<?= $this->config->item('ADMIN_BASE_URL'); ?>/pension/room/write/')" class="btn typeA-blue">신규</button>
                                </div>
                            </div>
                        </div>
                    </div>

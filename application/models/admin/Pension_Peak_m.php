...
Class Pension_Peak_m extends CI_Model {
    ...
    function lists($table , $type = '', $offset = '', $limit = '', $search_word = '') {
        ...
        if ($search_word != '') {
            // 검색어 있을 경우
            $sword = ' WHERE peak_title like "%' . $search_word . '%"';
        }
    ...
    function insert($arrays) {
        ...
        //종료시간을 다시 셋팅
        $end_date_time = $arrays['e_date']." 23:59:59";

        $insert_array = array(
            'peak_title' => $arrays['peak_title'],
            'peak_type' => $arrays['peak_type'],
            's_date' => $arrays['s_date'],
            'e_date' => date($end_date_time),            
